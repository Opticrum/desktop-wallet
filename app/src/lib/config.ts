// Frontend config constants + local helpers.
//
// Kept on the frontend per `docs/ipc/ipc-api.md` §6/§5: the enum option lists
// are static UI metadata (never in IPC), `defaultNodeConfig` is the local
// "Reset config" fallback (no command), and `serializeConfigYaml` /
// `detectChainFromRpc` are frontend formulas. The authoritative config
// load/save contract is `node.get_config` / `node.save_config`.

import type { Chain, NodeConfig, Service, ScriptCellDep } from '../api/types'
...
export function detectChainFromRpc(rpcUrl: string): Chain | null {
...
function serializeCellDeps(lines: string[], deps: ScriptCellDep[], indent: string): void {
  for (const dep of deps) {
    if (dep.kind === 'type_id') {
      lines.push(`${indent}- type_id:`)
      lines.push(`${indent}  code_hash: ${dep.code_hash}`)
      lines.push(`${indent}  hash_type: ${dep.hash_type}`)
      lines.push(`${indent}  args: ${dep.args}`)
    } else {
      lines.push(`${indent}- cell_dep:`)
      lines.push(`${indent}  out_point:`)
      lines.push(`${indent}    tx_hash: ${dep.tx_hash}`)
      lines.push(`${indent}    index: ${dep.index}`)
      lines.push(`${indent}  dep_type: ${dep.dep_type}`)
    }
  }
}
...
export function serializeConfigYaml(c: NodeConfig): string {
  ...
  for (const s of c.scripts) {
    ...
    serializeCellDeps(lines, s.cell_deps, '        ')
  }
  ...
  for (const u of c.udt_whitelist) {
    ...
    if (u.cell_deps && u.cell_deps.length > 0) {
      lines.push('      cell_deps:')
      serializeCellDeps(lines, u.cell_deps, '        ')
    }
