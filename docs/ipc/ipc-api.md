...
- `CommandError` 是 serde-tagged 枚举，前端按 `{ code, message }` switch：变体与 `SdkError` 一一对应（`Chain`、`Scan`、`Build`、`InvalidInput`、`AlreadyExhausted`、`NotExhausted`、`NotAuthorized`），外加应用级变体：`WalletLocked`、`NodeNotRunning`、`Node`、`InsufficientFunds`、`AlreadyExists`、`Config{message}`、`Io{path, message}`、`Internal{message}`。
...
| 域 | 命令 |
|---|---|
| wallet | `get_summary` `get_addresses` `get_transactions` `unlock` `lock` `create_hd_wallet` `import_mnemonic` `import_private_key` `derive_addresses` `send_ckb` |
| node | `get_runtime` `start` `stop` `get_logs` `get_config` `save_config` |
| channels | `list` `connect_peer` `disconnect_peer` `open_channel` `close_channel` |
| liquidity | `get_dashboard` `get_orders` `get_matches` `get_matches_near_exhaustion` `publish_order` `cancel_order` `inject_deposit` `withdraw_deposit` `extract_spent_match` |

### 4.1 wallet 域

| 命令 | 参数 | 返回 | 错误 | 说明 |
|---|---|---|---|---|
| `wallet.get_summary` | 无 | `WalletSummary`（新 wire type） | `Chain` | `WalletService.get_hd_wallet_balance` + `WalletSessionManager.status` + address.rs 当前 HD 地址（hrp 随 chain）。`unlocked=false` 是状态字段、非错误；`has_wallet=false`（首次运行，尚无 keystore）时前端进入创建/导入流程。`fiat_usd` 来自可选价格源（`None` → 前端隐藏 USD 行；替换 mock 的 `wallet.fiatUsd`）。`*_ckb` 已在 Rust 端由 shannons/1e8 预换算。 |
...
| `wallet.get_transactions` | `limit?: u32`、`offset?: u32` | `Vec<WalletTx>`（新 wire type） | `Chain`、`Scan` | 桌面账户历史索引器：`get_cells_by_lock_arg`（钱包 lock hashes）→ 每 cell `get_transaction` → Rust 端分类 `kind`（fiber 通道开/关经 funding-lock args 长度 20 / 通道 outpoint 识别）。前端保留 `typeCounts` 归约 + `activeTypes` 过滤 + `shortHash`/`truncatedHash`（公式，设计决策 #2）。 |
| `wallet.unlock` | `password: string`、`label?: string` | `WalletSummary`（新 wire type，`unlocked=true`） | `InvalidInput`（密码错误）、`Io`、`Chain` | `unlock_keystore`（AES-256-GCM 解密）+ `WalletSessionManager.create` —— RAM-only 会话，TTL 3600s 滑动。`label` 用于多 keystore 时选钱包。 |
...
| `liquidity.inject_deposit` | `match_outpoint: string`、`amount_shannons: u64`（前端 CKB→shannons） | `{ tx_hash }`（内联） | `WalletLocked`、`NotAuthorized`、`InvalidInput`、`Build`、`Chain` | `build_update_match`（buyer, `match_info`, `new_xudt_amount`, `capacity_delta=+amount`）。前端乐观地把 `depositCkb`/`withdrawableCkb` 平移同 delta，随后重拉 `get_matches` 取权威状态。 |
| `liquidity.withdraw_deposit` | `match_outpoint: string`、`amount_shannons: u64`（前端 CKB→shannons） | `{ tx_hash }`（内联） | `WalletLocked`、`NotAuthorized`、`InvalidInput`（金额 > `withdrawable_ckb`）、`Build`、`Chain` | `build_update_match`，`capacity_delta=−amount`。后端校验 `amount ≤ withdrawable`（→ `InvalidInput`）；前端额外把表单上限钳在 `withdrawableCkb`。本地负向平移 `deposit_ckb`/`withdrawable_ckb` 后重拉。 |
...
|---|---|---|
| `WalletSummary` | `{ has_wallet: bool, unlocked: bool, address: string, available_ckb: f64, total_ckb: f64, locked_ckb: f64, fiat_usd: Option<f64>, chain: 'mainnet'\|'testnet' }` | `WalletService.get_hd_wallet_balance`（available/total/locked，Rust 端 shannons/1e8）+ `WalletSessionManager.status`（unlocked）+ address.rs 当前 HD 地址（hrp 随 chain）。`has_wallet`（本地 keystore 是否存在）区分首次运行三态：无钱包 → 创建/导入；有钱包未解锁 → 输入密码；已解锁。`fiat_usd: Option<f64>` 来自可选价格源（外部，不在 SDK 中；`None` 隐藏 USD 行；替换 mock `wallet.fiatUsd`）。 |
...
| `WalletTx` | `{ id: string, kind: 'receive'\|'send'\|'channel_open'\|'channel_close', amount_ckb: f64（有符号：+receive/inbound，−send/outbound）, timestamp_ms: u64, tx_hash: string }` | 新桌面账户历史索引器：`get_cells_by_lock_arg`（钱包 lock hashes）→ 每 cell `get_transaction` → Rust 端分类 `kind`（fiber 通道开/关经 funding-lock args 长度 20 / 通道 outpoint 识别）。`amount` 由 CellOutput capacity 增量得出；`timestamp_ms = get_block_timestamp`（区块头 ts 即 ms）。替换 mock `wallet.txs`。 |
| `NodeRuntime` | `{ running: bool, alias: Option<string>, uptime_hours: u32, fiber_pubkey: string, fiber_addr: Option<string>, addresses: string[], chain: 'mainnet'\|'testnet', version: Option<string>, commit_hash: Option<string>, peers_count: u32, channel_count: u32, pending_channel_count: u32, watchtower: { mode: 'builtin'\|'standalone'\|'disabled', endpoint: Option<string> } }` | `FiberNodeInfo`（`node_info` RPC：version、commit_hash、pubkey→fiber_pubkey、node_name→alias、addresses、chain_hash；channel/pending/peers 计数为 `'0x'` hex 字符串解析为 u32）+ 桌面子进程状态（`running`、`uptime_hours = (now − process_start_ts)/3600`，停止时为 0）+ 持久化配置 chain（`setChain`）+ watchtower 从配置派生（`standalone_watchtower_rpc_url`→standalone；否则 `disable_built_in_watchtower`→disabled；否则 builtin；endpoint = 该 URL）。替换 mock `nodeRuntime` + `nodeWatchtower`。mock 遗留字段 `tipHeight`/`peers`/`cpuPercent`/`memPercent`/`synced` 当前未渲染：`tipHeight` 归 Phase 2 `node.tip_changed`；其余无替代，建议从 mock 删除（不进 wire）。watchtower 值迁移见 §4.2。 |
...
| `LiquidityMatch` | `{ outpoint: string, channel_outpoint: string, channel_capacity_ckb: f64, shannons_per_block: u64, annual_yield_bps: f64, deposit_ckb: f64, withdrawable_ckb: f64, xudt_amount: u128, created_at_ms: u64, expires_at_ms: u64, is_exhausted: bool, health: 'healthy'\|'warning'\|'critical'\|'exhausted', last_extraction_block: u64, projected_exhaustion_block: u64, seller_lock_hash: string }` | `MatchDetail`/`MatchSummary`（SDK：outpoint、channel_outpoint、`remaining_capacity_ckb`→channel_capacity_ckb（耗尽时 0）、shannons_per_block、annual_yield_bps、is_exhausted、last_extraction_block、projected_exhaustion_block 两类型皆有；**`health`（serde snake_case）、`seller_lock_hash` 仅 `MatchDetail`，`MatchSummary` 无此二字段**）+ `get_block_timestamp(match_current_block)`→created_at_ms（即 MatchDetail/MatchDeadline 的 `match_creation_block`）、`get_block_timestamp(projected_exhaustion_block)`→expires_at_ms（`shannons_per_block==0` 时为 `u64::MAX`）+ 桌面本地 sidecar `deposit_ckb`（回退 = `withdrawable_ckb`）。`withdrawable_ckb` = 当前 buyer-withdrawable 值（CKB 匹配为 `MatchInfo.ckb_capacity/1e8`，xUDT 为 `xudt.amount/1e8`）。前端由 `created_at_ms`/`expires_at_ms` + 原始字段计算 `matchLife`/`rentalDaysForMatch`/`daysLeft`/`computeInboundSummary`/donut slices（`OverviewChart`；`splitPct` 为池级汇总，见 §6）（公式，设计决策 #2）。**MatchHealth 大小写迁移**：mock `matchLife()` 当前返回大写 label（`'Healthy'` 等，mock/liquidity.ts:93-94），`LiquidityCellField` 的 badge 按大写 key 匹配；接入 wire 后须与小写 `health`（`'healthy'\|'warning'\|'critical'\|'exhausted'`）对齐。 |
...
| 流动性 | `totalDemand` / `avgApy` / `pending` / `avgDwell` / `sharePct`（订单池汇总；`sharePct` = 订单需求占比，与 per-order 工具提示 share `hoveredOrderShare = channelCapacityCkb/totalDemand*100` 是不同的值） | `LiquidityOrder` 原始字段 |
| 流动性 | `splitPct = orderDemandTotal / (orderDemandTotal + matchCapTotal)`（订单需求 vs 匹配容量占比，池级汇总） | `LiquidityOrder` + `LiquidityMatch` 原始字段 |
...
| 节点 | `uptimeHours + 'h'` 展示 | `NodeRuntime.uptime_hours` |
