...
- `CommandError` 是 serde-tagged 枚举，前端按 `{ code, message }` switch：变体与 `SdkError` 一一对应（`Chain`、`Scan`、`Build`、`InvalidInput`、`AlreadyExhausted`、`NotExhausted`、`NotAuthorized`、**`WithdrawWindowExpired`、`HesitationNotElapsed`、`PartialWithdrawNotAllowed`、`InjectDuringHesitation`** —— 后四个为犹豫期窗口错误码），外加应用级变体：`WalletLocked`、`NodeNotRunning`、`Node`、`InsufficientFunds`、`AlreadyExists`、`UnsupportedNetwork`（主网 Opticrum 未开放等）、`Config{message}`、`Io{path, message}`、`Internal{message}`。
...
| 域 | 命令 |
|---|---|
| wallet | `get_summary` `get_status` `get_addresses` `get_transactions` `unlock` `lock` `create_hd_wallet` `import_mnemonic` `import_private_key` `derive_addresses` `send_ckb` `set_network` |
| node | `get_runtime` `start` `stop` `get_logs` `get_config` `save_config` `fnn_cli_status` `fnn_cli_open` `open_url` `list_targets` `add_external` `update_external` `remove_external` `set_active` |
...
| `wallet.get_status` | 无 | `WalletStatus`（新 wire type）：`{ has_wallet, unlocked, address, chain }` | 无 | **快速本地状态**（keystore 存在性 + 会话解锁 + keyring 首地址 + **当前钱包 CKB 网络**），**不做任何链上查询**。前端 `WalletModule` / `WalletNetworkContext` 用它在加载瞬间渲染解锁表单与网络指示，不等待 `get_summary` 的余额查询/交易回溯。`chain` 来自持久化的 `wallet-network.json`（默认 testnet）。 |
| `wallet.set_network` | `chain: 'mainnet' \| 'testnet'` | `WalletStatus` | `InvalidInput`、`Io`、`Chain` | **钱包网络热切换**（不重建 Fiber 节点）：持久化 `wallet-network.json` → 切换 CKB RPC/indexer → 用同一助记词按目标 HRP（`ckb`/`ckt`）重编码地址 → 交易/订单缓存按 chain 分桶。切换后 Opticrum 市场跟随钱包网络；主网市场写入返回 `unsupported_network`（合约未部署）。**不**停止内置 Fiber 节点；节点 `fiber.chain` 仍独立。钱包与节点网络不一致时，前端阻断 send / 开闭通道 / invoice / 流动性链上操作，保留检视、日志、启停、配置、peer 管理。 |
...
| `node.fnn_cli_open` | `url: string`（RPC URL。内置：`node.get_config` 的 `rpc.listening_addr` 经前端 `fiberRpcUrl` 派生；外部：创建该节点时填写的 URL，经 `withHttpScheme` 补 `http://`） | `null` | `Io` | 打开终端窗口运行 `fnn-cli -u <url>` 并保持会话（macOS Terminal.app via osascript；Linux `x-terminal-emulator`；Windows `cmd /k`）。内置与外部节点的 Fiber 端口行均可触发。 |
| `node.open_url` | `url: string` | `null` | `Io` | 用系统默认浏览器打开 URL（`open` / `xdg-open` / `start`）。fnn-cli 未安装时的安装引导跳转。 |
| `node.list_targets` | 无 | `NodeTargetList` | 无 | 内置节点 + 已保存的外部 Fiber RPC 列表，以及当前 `activeId`。`builtin.running/starting` 永远是**本机内置进程**（托盘用这个，不跟选中目标走）。 |
| `node.add_external` | `alias: string`、`rpcUrl: string`、`authToken?: string` | `NodeTargetList` | `InvalidInput`、`Chain`（探活失败/超时） | 先 `node_info` 探活（8s 超时），成功才写入 `node-targets.json`。不自动切换；前端再调 `set_active`。 |
| `node.update_external` | `id`、`alias`、`rpcUrl`、`authToken?: string` | `NodeTargetList` | `InvalidInput`、`Chain` | 不能改 `builtin`。若正在编辑当前激活目标，探活成功后立刻换 client。 |
| `node.remove_external` | `id: string` | `NodeTargetList` | `InvalidInput` | 不能删 `builtin`。若删的是当前激活目标，自动切回内置。 |
| `node.set_active` | `id: string`（`"builtin"` 或外部 id） | `NodeRuntime` | `InvalidInput` | 热切换共享 Fiber RPC client；更新流动性用的 `fiber_pubkey`。内置进程不停。 |
...
| `WalletStatus` | `{ has_wallet: bool, unlocked: bool, address: string, chain: 'mainnet'\|'testnet' }` | `get_status` / `set_network` —— 仅本地（keystore + 会话 + 首地址 + 持久化钱包网络），无链上查询。`chain` 驱动地址 HRP 与 Opticrum 市场；前端 TopBar / 钱包切换器读取它。 |
...
| `NodeRuntime` | `{ running: bool, starting: bool, alias: Option<string>, started_at_ms: Option<u64>, uptime_hours: u32, fiber_pubkey: string, fiber_addr: Option<string>, addresses: string[], chain: 'mainnet'\|'testnet', version: Option<string>, commit_hash: Option<string>, peers_count: u32, channel_count: u32, pending_channel_count: u32, watchtower: { mode: 'builtin'\|'standalone'\|'disabled', endpoint: Option<string> }, kind: 'builtin'\|'external', target_id: string, rpc_url: string }` | 跟随**当前选中目标**。内置：`node_info` + 本机进程状态（`rpc_url` 为空，前端从 config 派生 Fiber 端口）。外部：只 ping 远程 `node_info`（`running` = 可达；无 uptime / watchtower / 启停）；`rpc_url` 为创建该外部节点时填写的 Fiber RPC URL。`kind`/`targetId` 供前端藏启停与日志。 |
...
| `NodeTargetList` | `{ active_id: string, builtin: { id, alias, running, starting, watchtower }, externals: Array<{ id, alias, rpc_url, auth_token? }> }` | `node-targets.json` + 本机内置进程状态。托盘只读 `builtin`。 |
...
| `Channel` | `{ channel_id: string, tx_hash: string, output_index: u32, capacity_ckb: f64, capacity_shannons: u64, local_balance_ckb: f64, local_balance_shannons: u64, remote_balance_ckb: f64, remote_balance_shannons: u64, state: string, state_flags: Option<string>, is_public: bool, enabled: bool, created_at_ms: u64, close_flags: Option<u32>, base_fee_mshannons: Option<u64>, fee_rate_ppm: Option<u64> }` | `FiberChannelInfo`（链上原始：channel_id、tx_hash、output_index、capacity/local/remote balance 均 shannons、`state_name`→state、`state_flags`→state_flags、is_public、enabled、created_at ms、close_flags）。`*_ckb = shannons/1e8`（Rust 端）。`state` 为 fiber 原始 state_name（NegotiatingFunding/…/ChannelReady/ShuttingDown/Closed）；`state_flags` 为 fiber 的 SCREAMING_SNAKE 标志串（如 `AWAITING_REMOTE_TX_COLLABORATION_MSG`），`ChannelReady` 为 `null`。前端映射到 active\|pending\|closing 显示桶，badge 文案按 state_name 细分。`base_fee_mshannons`/`fee_rate_ppm` 来自 fiber channel-detail RPC（`get_channel_info`）可用时 —— 不在 `FiberChannelInfo` 中（已记录缺口；今日仅 mock）。 |
