...
| pool cell match | `outpoint, channelOutpoint, annualYieldBps, channelCapacityCkb, createdAt, expiresAt, depositCkb, originalStakeCkb, withdrawableCkb, hesitationEndsAtMs, role, matchPhase{hesitating/active/exhausted}, matchLife{pct,label,isExhausted}, extractionProgress{pct,...}` | `liquidity.get_matches` + 前端 `matchLife`/`matchPhase`/`extractionProgress`/`hesitationRemainingMs`/`rentalDaysForMatch`/`lifeTierColor`（犹豫期 cell 用 `--violet` 徽章 + h/m 倒计时；已提取的 cell 底部 rim 显示「已提取 X%」迷你进度徽章） |
...
| sheet match | `matchLife, matchPhase, role, hesitationEndsAtMs, originalStakeCkb, extractionProgress, channelCapacityCkb, withdrawableCkb, channelOutpoint, outpoint, annualYieldBps, shannonsPerBlock, depositCkb, createdAt, expiresAt`（犹豫期内：倒计时 hero + 「放弃订单并提取全部租金」单按钮；犹豫期外：仅「注入」单按钮；**抽离/注入互斥、每次只显示一个**，按钮下方显示犹豫期状态行；时间 life bar 下另显示**提取进度**区块：进度条 + 原始质押/已提取/剩余三列） | |
...
| 表单 | publish（`channelCapacityCkb, shannonsPerBlock, depositCkb, rentalDays=30, fiberAddress?`）；inject（金额输入，无上限）；abandon = 犹豫期内「放弃订单」直接二次确认（全额提取，无金额输入） | |
...
4b. `matchPhase(match, now)`：`isExhausted→'exhausted'`；`role≠'buyer'` 或 `lastExtractionBlock>0→'active'`；`now < hesitationEndsAtMs→'hesitating'`；否则 `'active'`（近似链上 `tip−match_creation_block ≤ 3600 && last_extraction_block==0`）；配套 `hesitationRemainingMs` + `formatDurationHm`（h/m 倒计时）。犹豫期 cell 用 `--violet` 徽章 + 倒计时；抽屉动作互斥：窗口内仅「放弃订单并提取全部租金」、窗口外仅「注入」，按钮下方为犹豫期状态行
4c. `extractionProgress(match)` = `{ originalCkb: originalStakeCkb, remainingCkb: depositCkb, extractedCkb: max(0, original−remaining), pct: round(extracted/original×100) }` —— 实际租金提取进度（容量口径）；`matchLife` 仍为时间口径。cell 底部 rim「已提取 X%」迷你进度徽章（pct>0 时显示）；抽屉 time life bar 下显示完整进度条 + 原始/已提取/剩余三列
...
| 7 | `liquidity.get_matches` 返回 `deposit_ckb / original_stake_ckb / withdrawable_ckb / created_at_ms / expires_at_ms` + **犹豫期字段**（`match_creation_block / hesitation_ends_at_ms / role`；SDK 只有块序号） | **new-core-crate** | `MatchSummary` 只有 `remaining_capacity_ckb/last_extraction_block/projected_exhaustion_block`，连 `match_creation_block` 都没有（只在 `MatchDetail`/`MatchDeadline`）；`deposit_ckb/withdrawable_ckb` 在 `MatchInfo.ckb_capacity`（opticrum-calculator 原始字段，SDK 未暴露为聚合）；block→ms 时间戳 SDK 无（`get_block_timestamp` 只在 rust-server ChainProvider，SDK 无）；**原始质押链上不存**（`MatchData` 无此字段），SDK 无回溯能力 | new-core-crate 写 `get_matches` 服务：`scan_matches` → 每 `MatchInfo` 调 `get_block_timestamp(match_current_block)→created_at_ms`、`get_block_timestamp(projected_exhaustion_block)→expires_at_ms`（`shannons_per_block==0`→`u64::MAX`）；`deposit_ckb`=`MatchInfo.ckb_capacity/1e8`（当前剩余租金池）；**`original_stake_ckb` = 交易图回溯**：沿 match cell producing-tx 的 Opticrum 锁输入（133B match / 65B order）回退到 `order_match` 创建 tx，其 match 输出容量 − occupied（occupied = `last_raw − ckb_capacity`，跨化身恒定），按 match args hex 缓存，回溯失败回退 `deposit_ckb`；**`withdrawable_ckb`=全额质押仅当钱包为买方且犹豫窗口内（`last_extraction_block==0 && tip−match_creation_block ≤ 3600`），否则 `0.0`**；新增 `match_creation_block`（=`match_current_block`）、`hesitation_ends_at_ms`（=`created_at_ms + HESITATION_BLOCKS×12s`）、`role`（buyer/seller lock hash 与钱包 lock hash 比对 → `buyer`/`seller`/`other`）；`channel_capacity_ckb`= funding 通道容量（`match_channel_capacity`）。前端 `matchLife/matchPhase/extractionProgress/hesitationRemainingMs/rentalDaysForMatch/daysLeft/computeInboundSummary` 仍按 #2 留前端 |
...
**解决路径**：
- **`get_matches`**（new-core-crate）：`scan_matches` → 每 `MatchInfo` 调 `get_block_timestamp(match_current_block)→created_at_ms`、`get_block_timestamp(projected_exhaustion_block)→expires_at_ms`（`shannons_per_block==0→u64::MAX`）；`deposit_ckb`=`MatchInfo.ckb_capacity/1e8`（当前剩余租金池）；`original_stake_ckb`=交易图回溯（见 row 7，按 match args 缓存）；`withdrawable_ckb`=全额质押仅当钱包为买方且犹豫窗口内，否则 `0.0`；新增 `match_creation_block`/`hesitation_ends_at_ms`/`role`（见 row 7）；`channel_capacity_ckb`= funding 通道容量（`match_channel_capacity`）。前端 `matchLife/matchPhase/extractionProgress/hesitationRemainingMs/rentalDaysForMatch/daysLeft/computeInboundSummary` 按 #2 留前端。
...
- **liquidity 写操作**：`publish/cancel/inject/withdraw/extract` 包一层 assemble + `balance_and_sign` + send_and_wait（复用 `TransactionAssembler` 模式，300s 确认超时）；`publish_order` 写 sidecar（`rental_days/created_at_ms/deposit_ckb`）；extract 用 `MatchInfo.ckb_capacity/1e8` 计算 `returned_ckb` 返回（toast `lmExtractDeleted` 显示）。注意：extract 仅 `life.isExhausted` 可用（前端 gate）；inject 不设上限（`POSITIVE_INFINITY`），**犹豫期内被 SDK 拒绝（`InjectDuringHesitation`）**；**withdraw = 犹豫期内买方「放弃订单」全额 dump**（`capacity_delta=−ckb_capacity`、`new_xudt=0`，SDK 窗口外返回 `WithdrawWindowExpired`），前端用直接二次确认（无金额输入）。
