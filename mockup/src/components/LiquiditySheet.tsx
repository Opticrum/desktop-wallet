...
function formatCkbPerBlock(shannons: number): string {
...
function truncateOutpointNoIndex(outpoint: string): string {
...
export function LiquiditySheet({
  ...
  if (target.kind === 'order') {
    ...
    const dwellH = dwellHours(order.createdAt)
    ...
    <div className="lm-drawer" aria-label={t.mgDetails} style={{ '--tier': lifeTierColor(life.pct) } as CSSProperties}>
      ...
      <div className="mg-life ms-life">
        ...
        <span className="mg-life-text">
          ...
          <span className="ms-life-expiry">{formatTimestamp(match.expiresAt)}</span>
        </span>
      </div>
