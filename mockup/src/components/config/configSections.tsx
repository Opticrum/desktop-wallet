...
import { HASH_TYPES, RPC_MODULES, SERVICES } from '../../mock/fiberConfig'
...
export function CkbSection() {
  ...
  return (
    <Section title={t.cfgSectionCkb} icon={<IconLink />}>
      ...
      {config.udtWhitelist.length === 0 && <div className="config-empty">{t.cfgUdtEmpty}</div>}
      {config.udtWhitelist.map((u, i) => (
        <ItemCard
          key={i}
          head={
            <input
