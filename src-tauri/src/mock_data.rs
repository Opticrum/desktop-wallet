...
pub fn mock_wallet() -> (WalletSnapshotData, Vec<WalletTx>) {
  ...
  let txs = vec![
    tx("tx1", WalletTxKind::Receive, 500.0, 1_785_201_240, "0x7a1c9e2b4d8f01a3c5e7b9d0f2a4c6e8b1d3f5a7c9e0b2d4f6a8c0e2b4d6f8a0"),
    tx("tx2", WalletTxKind::ChannelOpen, -1200.0, 1_785_139_320, "0x91b044aa12cd34ef56ab78cd90ef12ab34cd56ef78ab90cd12ef34ab56cd78ef"),
    tx("tx3", WalletTxKind::Send, -42.5, 1_785_037_200, "0x33de0c18a1b2c3d4e5f60718293a4b5c6d7e8f90123456789abcdef012345678"),
    tx("tx4", WalletTxKind::Receive, 88.25, 1_784_981_880, "0x55aa11bb22cc33dd44ee55ff66778899aabbccddeeff00112233445566778899"),
    tx("tx5", WalletTxKind::ChannelClose, 640.12, 1_784_851_500, "0xabcdef0123456789fedcba9876543210abcdef0123456789fedcba9876543210"),
    tx("tx6", WalletTxKind::Send, -250.0, 1_784_786_700, "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"),
    tx("tx7", WalletTxKind::Receive, 1200.0, 1_784_683_980, "0xfedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210"),
    tx("tx8", WalletTxKind::ChannelOpen, -800.0, 1_784_629_320, "0x0f1e2d3c4b5a69788796a5b4c3d2e1f0a9b8c7d6e5f4a3b2c1d0e9f8a7b6c5d4e3"),
    tx("tx9", WalletTxKind::Send, -15.75, 1_784_504_880, "0x11223344556677889900aabbccddeeff11223344556677889900aabbccddeeff"),
    tx("tx10", WalletTxKind::Receive, 320.5, 1_784_466_600, "0xccddeeff00112233445566778899aabbccddeeff00112233445566778899aabb"),
  ];
...
pub fn mock_runtime() -> NodeRuntime {
  NodeRuntime {
    ...
    fiber_addr: Some("/ip4/18.142.44.12/tcp/8115".to_string()),
    ...
    nodes: vec![
      ChannelNode {
        peer: PeerInfo {
          ...
          addr: Some("/ip4/104.21.88.201/tcp/8115".into()),
        },
        channels: vec![
          chan("ch-01", "0x8f3a…1c40", 2000.0, 1250.4, 749.6, "ChannelReady", 1000, 120, created),
          chan("ch-02", "0x2b91…a7d2", 1500.0, 620.0, 880.0, "ChannelReady", 800, 90, created),
        ],
      },
      ChannelNode {
        peer: PeerInfo {
          ...
          addr: Some("/ip4/47.98.210.66/tcp/8115".into()),
        },
        channels: vec![
          chan("ch-03", "0x44f0…9e21", 800.0, 400.0, 400.0, "NegotiatingFunding", 1000, 100, created),
          chan("ch-04", "0xd13c…55f8", 600.0, 510.2, 89.8, "ChannelReady", 500, 75, created),
        ],
      },
      ChannelNode {
        peer: PeerInfo {
          ...
          addr: Some("/ip4/13.229.101.7/tcp/8115".into()),
        },
        channels: vec![chan("ch-05", "0x7be9…2a04", 420.0, 180.5, 239.5, "ChannelReady", 700, 110, created)],
      },
      ChannelNode {
        peer: PeerInfo {
          id: "n-relay-eu".into(),
          alias: Some("relay-eu".into()),
          addr: Some("/ip4/65.21.103.44/tcp/8115".into()),
        },
        channels: vec![chan("ch-06", "0xa09c…33d7", 1000.0, 350.0, 650.0, "ChannelReady", 1200, 150, created)],
      },
      ChannelNode {
        peer: PeerInfo {
          id: "n-lp-foundation".into(),
          alias: Some("lp-foundation".into()),
          addr: Some("/ip4/34.96.140.55/tcp/8115".into()),
        },
        channels: vec![chan("ch-07", "0x55d1…8f6a", 300.0, 120.0, 180.0, "ShuttingDown", 1000, 100, created)],
      },
      ChannelNode {
        peer: PeerInfo {
          id: "n-merchant-asia".into(),
          alias: Some("merchant-asia".into()),
          addr: Some("/ip4/13.213.4.99/tcp/8115".into()),
        },
        channels: vec![],
      },
      ChannelNode {
        peer: PeerInfo {
          ...
          addr: Some("/ip4/13.115.32.211/tcp/8115".into()),
        },
        channels: vec![],
      },
    ],
...
pub fn mock_orders() -> Vec<LiquidityOrder> {
  vec![
    order("0xa1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2:0", 50_000.0, 100_000, 500.0, 30, 1_785_289_217),
    order("0xb2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3:1", 35_000.0, 80_000, 350.0, 30, 1_785_380_528),
    order("0xc3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4:2", 42_000.0, 60_000, 420.0, 14, 1_785_479_201),
    order("0xd4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5:0", 25_000.0, 90_000, 250.0, 7, 1_785_545_733),
    order("0xe5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6:0", 60_000.0, 110_000, 600.0, 30, 1_786_241_692),
    order("0xf6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7:1", 30_000.0, 70_000, 300.0, 14, 1_786_178_829),
  ]
}
...
fn match_(
  ...
  LiquidityMatch {
    ...
    seller_lock_hash: "0x8e55773c1c3f5b2f1f2f6e9a8d0c5b4a3f2e1d0c9b8a7f6e5d4c3b2a1f0e9d8c".to_string(),
  }
}
...
pub fn mock_deadlines(matches: &[LiquidityMatch]) -> Vec<MatchDeadline> {
  let mut deadlines: Vec<MatchDeadline> = matches
    ...
    .map(|m| MatchDeadline {
      ...
      projected_exhaustion_block: m.projected_exhaustion_block,
      blocks_remaining: if m.health == MatchHealth::Critical { 25_000 } else { 300_000 },
      estimated_hours_remaining: if m.health == MatchHealth::Critical { 83 } else { 1000 },
      health: m.health,
